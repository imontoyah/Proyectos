...
def to_excel_VND(result, lista):
  # Export the DataFrame to an Excel file
  writer = pd.ExcelWriter('mtVRP_IsabellaMontoya_VND.xlsx', engine='xlsxwriter')
  for i, df in enumerate(result):
      df.to_excel(writer, sheet_name=lista[i], na_rep='', header=False, index=False)
  writer.save()

def to_excel_VND_mixed(result, lista):
  # Export the DataFrame to an Excel file
  writer = pd.ExcelWriter('mtVRP_IsabellaMontoya_VND_mixed.xlsx', engine='xlsxwriter')
  for i, df in enumerate(result):
      df.to_excel(writer, sheet_name=lista[i], na_rep='', header=False, index=False)
  writer.save()