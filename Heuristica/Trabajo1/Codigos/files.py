...
def dataFrame(results, Th, Th_dist, route, run_time, R):
  ...
  for i in range(R):
    data.append(route[i] + [Th_dist[i], cons_len[i]])
...
def to_excel_constr(result, lista):
  # Export the DataFrame to an Excel file
  writer = pd.ExcelWriter('mtVRP_IsabellaMontoya_Constructive.xlsx', engine='xlsxwriter')
  for i, df in enumerate(result):
      df.to_excel(writer, sheet_name=lista[i], na_rep='', header=False, index=False)
  writer.save()

def to_excel_noising(result, lista):
  # Export the DataFrame to an Excel file
  writer = pd.ExcelWriter('mtVRP_IsabellaMontoya_Ruido.xlsx', engine='xlsxwriter')
  for i, df in enumerate(result):
      df.to_excel(writer, sheet_name=lista[i], na_rep='', header=False, index=False)
  writer.save()

def to_excel_grasp(result, lista):
  # Export the DataFrame to an Excel file
  writer = pd.ExcelWriter('mtVRP_IsabellaMontoya_GRASP.xlsx', engine='xlsxwriter')
  for i, df in enumerate(result):
      df.to_excel(writer, sheet_name=lista[i], na_rep='', header=False, index=False)
  writer.save()
